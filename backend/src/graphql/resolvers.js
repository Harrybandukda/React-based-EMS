...
const rootResolvers = {
  Query: {
    ...
    searchEmployees: async (_, filters) => {
      // Create a search query object that only includes non-empty filters
      const searchQuery = {}

      // Only add department to query if it's provided and not empty
      if (filters.department) {
        searchQuery.department = filters.department
      }

      // Only add position to query if it's provided and not empty
      if (filters.position) {
        searchQuery.position = filters.position
      }

      // Return employees matching the search query
  ...
  Mutation: {
    ...
    login: async (_, { username, password }) => {
      ...
      const accessToken = jwt.sign({ userId: foundUser.id, username: foundUser.username }, process.env.JWT_SECRET, {
        expiresIn: "24h",
      })
